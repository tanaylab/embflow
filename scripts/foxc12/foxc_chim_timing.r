...
foxc_chim_timing = function(tag = "all",included_colors = NULL) {
  
  mat_nm = "foxc_chim_wt10"
  mc_id = "foxc_chim_wt10_recolored"
  cgraph_id =  mat_nm
  mat = scdb_mat(mat_nm)
  mc = scdb_mc(mc_id)
  ...
  ko_cls = chim_cls[( mat@cell_metadata[chim_cls,"cell_type"] == "KO" ) & ( mat@cell_metadata[chim_cls,"embryo"] %in% chim_embryos )]
  ...
  if(tag != "all") {
    file_nm =  sprintf("%s/time_dist_host_ko_%s.Rda",data_dir,tag)
  } else {
    file_nm =  sprintf("%s/time_dist_host_ko.Rda",data_dir)
  }
  
  save(time_dist_host_ko,file = file_nm)
...
foxc_chimera_plot_cumulative_distribution_ko_host = function(mat_nm,fig_dir = NULL,tag = "all",plot_pdf = F,chim_col = "#83c26d") {
  ...
  if(tag == "all") {
    ...
    load(sprintf("%s/time_dist_host_ko_%s.Rda",data_dir,tag))
  }
  ...
  for (i in 1:length(chim_embryos)) {
    ...
    ko_median = median(rep(c(1:153),ko_dist_all[chimera,]))
    
    delta_median[i] = host_median - ko_median
    
    ks_ko_host_chim = ks.test(x = rep(c(1:153),host_dist_all[chimera,]),y = rep(c(1:153),ko_dist_all[chimera,]))
    ks_statistic[i] = ks_ko_host_chim$statistic
    p_value[i] = ks_ko_host_chim$p.value
    ...
    if(!is.null(fig_dir)) {
      ...
      if (plot_pdf) {
        ...
        plot(c(xlim_min:xlim_max),host_dens[xlim_min:xlim_max],type = "l", main = paste0("FoxC DKO ",i),lwd = lwd,
             xlab = "Transcriptional rank",ylab = "",ylim = c(0,1),col = "gray30",cex.main = 3,cex.lab = 2,cex.axis = 2)
        
        lines(c(xlim_min:xlim_max),ko_dens[xlim_min:xlim_max],type = "l",lwd = lwd,col = "#83c26d")
        ...
        if(tag == "all") {
          fn = sprintf("%s/cumulative_time_dist_%s.png",fig_dir,chimera)
        } else {
          fn = sprintf("%s/cumulative_time_dist_%s_%s.png",fig_dir,chimera,tag)
        }
        png(fn,w = 500*fig_scale,h = 400*fig_scale)
        plot(c(xlim_min:xlim_max),host_dens[xlim_min:xlim_max],type = "l", main = paste0("FoxC DKO ",i),lwd = lwd,
             xlab = "Transcriptional rank",ylab = "",ylim = c(0,1),col = "gray30",cex.main = 3,cex.lab = 2,cex.axis = 2)
        
        lines(c(xlim_min:xlim_max),ko_dens[xlim_min:xlim_max],type = "l",lwd = lwd,col = chim_col)
